...
app.get('/', (req, res) => {
    res.render('index', { title: 'PDF to Excel Converter', message: 'Welcome to the premium converter!', error: null });
});
...
app.post("/upload", upload.array("pdf", 20), async (req, res) => {
    ...
    try {
        ...
        mainSheet.columns = [
            ...
            { header: "Contract Number", key: "contactNumber" },
        ...
        for (const file of req.files) {
            ...
            const clean = (val) => {
                ...
                let r = val.replace(/[\u0900-\u097F]/g, '').replace(/[|]/g, '').trim();
                if (r.length > 4 && /^\d+$/.test(r)) {
                    const half = r.length / 2;
                    if (Number.isInteger(half) && r.substring(0, half) === r.substring(half)) return r.substring(0, half);
                }
            ...
            const extract = (regex, source = text) => {
                ...
                return match ? clean(match[1]) : "Not Found";
            };
            ...
            mainSheet.addRow({ fileName: originalName, contactNumber, contractGeneratedDate, bidRaPbpNo, duration, amountOfContract });
        ...
        [mainSheet, orgSheet, buyerSheet, financeSheet, payingSheet, consigneeSheet, serviceProviderSheet, serviceDetailsSheet].forEach(styleSheet);

        const outputPath = `outputs/contracts-${Date.now()}.xlsx`;
        await workbook.xlsx.writeFile(outputPath);

        const downloadName = req.files.length === 1 ? `${req.files[0].originalname.replace('.pdf', '')}.xlsx` : 'contracts.xlsx';
